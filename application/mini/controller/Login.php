...
class Login extends Base
{
    ...
    public function wxMiniLogin()
    {
        ...
        db('member')->where(['id'=>$member['id']])->setField('m_wx_openId',$res['openid']);
        //获取token
        $token = $this->generateToken($member);
        //存储token
        Cache::set($token, $member, config('token_expire'));
    ...
    public function wxMiniRegister()
    {
        ...
        $mem=db('member')->where(['m_account'=>$param['account']])->find();
        ...
        if($mem){
            if($mem['m_wx_openId']){
                $this->ajaxError("该手机号已绑定");
            }
            $add['m_wx_openId']=$param['openid'];
            if(!$mem['m_fatherId']&&$invitation){
                $parent = db('member')->where(['m_invitation_code' => $invitation])->find();
                if (empty($parent)) {
                    $this->ajaxError('输入的邀请码有误');
                }
                $add['m_fatherId']        = $parent['id'];
                $add['m_grandpaId']       = $parent['m_fatherId'];
                $add['m_grate_grandpaId'] = $parent['m_grandpaId'];
                if (empty($parent["m_pathtree"])) {
                    $add["m_pathtree"] = ",{$parent["id"]},";
                } else {
                    $add["m_pathtree"] = "{$parent["m_pathtree"]}{$parent["id"]},";
                }
            }
            if(empty($mem['unionid'])){
                $add['unionid']=$param['unionid'];
            }
            db()->startTrans();
            $qt_sms = new SendSms();
            ...
            $res=db('member')->where(['id'=>$mem['id']])->update($add);
            if($res==false){
                db()->rollback();
                $this->ajaxError('绑定失败');
            }
            $token = $this->generateToken($mem);
            Cache::set($token, $mem, config('token_expire'));
            //返回token
            $mem["token"] = $token;
            //去除数组中的null值
...
//                if ($data['unionid']) {
                $member = db('member')
                    ->where('m_wx_openId', $data['openid'])
                    ->find();
                //小程序绑定openID
                if ($member && $type == "mini") {
                    db('member')
                        ->where(['id' => $member['id']])
                        ->setField('m_wx_openId', $data['openid']);
                }
//                } else {
//                    $member = [];
//                }
    ...
    public function ThirdPartyRegister()
    {
        if (request()->post()) {
            ...
            if ($type == 1) {
                //微信个人信息
                if ($wx_type == "mini") {
                    $wx_user = $this->WxDecode($data1);
                } else {
                    $wx_user = $this->getWxUser($code);
                }
                $data['m_wx_openId'] = $wx_user['openid'];
                ...
                $AliPay   = new Alipay($this->config);
                $ali_user = $AliPay->aliLogin($code);
                if ($ali_user['code'] != 10000) {
                    $this->ajaxError('支付宝登录失败');
                }
                $data['m_ali_token'] = $ali_user['user_id'];
                $data['m_thumb']     = $ali_user['avatar'];
                $data['m_nickname']  = $ali_user['nick_name'];
                //未知的登陆方式
            } else {
                $this->ajaxError('未知错误');
            }
