...
class Jpush extends Base
{
    ...
    {
        //获取app_key
        $this->app_key = getSettings('Jpush', 'app_key');
        //获取master_secret
        $this->master_secret = getSettings('Jpush', 'master_secret');
    }
    ...
    public function send_to_one($account, $alert, $title, $extras)
    {
        $notification = [
            'title'      => $title,//通知栏标题
            'builder_id' => 1, //通知栏样式
            'extras'     => json_encode($extras),//扩展字段 这里自定义 JSON 格式的 Key / Value 信息，以供业务使用。
        ];
