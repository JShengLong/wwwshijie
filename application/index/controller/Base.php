...
//            header('Access-Control-Allow-Origin: http://localhost:8080');
    ...
    public function uploadVideo(){
        ...
        if (empty($fileName)) $this->ajaxError('请选择视频');
        // 要上传图片的本地路径
        $filePath = $fileName->getRealPath();
        halt($fileName->getInfo('name'));
        $oss=new Alioss();
        halt($oss->oss($filePath));
    }
    /**
     * @param $code
     * @param $msg
     ...
     * @return array
     */
    public function getWxUser($datas)
    {
...
//        halt($secret);
        $get_token_url = "https://api.weixin.qq.com/sns/oauth2/access_token?appid={$appid}&secret={$secret}&code={$datas['code']}&grant_type=authorization_code";
        $res      = $this->httpGet($get_token_url);
        $json_obj = json_decode($res, true);
        if(!isset($json_obj['access_token'])){
            $this->ajaxError('code值错误');
        }
