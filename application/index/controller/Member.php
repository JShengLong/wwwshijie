...
class Member extends Signin
{
    ...
    public function member()
    {
        if (request()->isPost()) {
            ...
            $member['m_fatherId'] = db('member')
                ->where(['id' => $member['m_fatherId']])
                ->value('m_invitation_code');
            //返回个人信息
    ...
    public function myRecommend(Request $request)
    {
        if ($request->isPost()) {
            ...
            $member                 = db('member')
                ->where(['id' => $this->uid])
                ->find();
            $return['integral_num'] = number_format($member['m_integral_num'], 2);
            //分页
            $page = input('page', 1);
            //是否采购
            $isbuy = input('isbuy', 2);
        ...
        if ($request->isPost()) {
            ...
            foreach ($order['order_detail'] as $k => $v) {
                $order['order_detail'][$k]['r_id'] = db('refund')
                    ...
                    ->order('id desc')
                    ->value('id');
                if ($order['order_detail'][$k]['r_id'] == null) {
                    $order['order_detail'][$k]['r_id'] = 0;
                }
        ...
        if ($request->isPost()) {
            ...
            if ($return['order']['o_distribution_mode'] == 2) {
                ...
                if ($return['order']['o_isexp'] == 1) {
                    ...
                    //物流单号
                    $param['num'] = trim($return['order']['o_expresssn']);
                    //手机号
...
//                    halt($order_detail['d_total'].'-'.$order['o_ptotal']);
            ...
            if($order['o_status']==2){
                $order_detail['r_total']= $total+$order['o_freight'];
            }else{
            ...
            $order_detail['status']     = $order['o_status'];
    ...
    public function refund(Request $request)
    {
        if ($request->isPost()) {
            ...
            //退款积分
            $integral  = 0;
            $integrals = getSettings('integral', 'integral');
            //判断是否有积分抵扣
            if ($order['o_integral'] > 0) {
                //判断是否是全部抵扣
                if (($order['o_actual_payment'] - $order['o_freight']) > 0) {
