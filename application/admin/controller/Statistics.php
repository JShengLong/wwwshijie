...
class Statistics extends Right implements curdInterface
{
    /**
     * 特别说明
     * Common中的文件不能直接修改！！！！
    ...
    protected $modelName  = 'Statistics';  //模型名,用于add和update方法
    protected $indexField = [];  //查，字段名
    protected $addField   = ['name','type','info','sql','createTime','updateTime','stort'];    //增，字段名
    protected $editField  = ['name','type','info','sql','createTime','updateTime','stort'];   //改，字段名
    /**
    ...
    protected $orderField = 'stort asc';  //排序字段
...
//        'info|详情' => 'require',
    ...
    public function change_status(){
        $id=input('id');
        $list=db('statistics')->where(['id'=>$id])->find();
        echo $list['sql'];
    }
